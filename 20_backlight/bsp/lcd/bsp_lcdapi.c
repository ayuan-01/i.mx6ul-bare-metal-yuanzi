...
/*
 * @description	: 显示指定的16进制数字，高位为0的话不显示
 * @param - x	: 起始坐标点X轴。
 * @param - y	: 起始坐标点Y轴。
 * @param - num	: 数值(0~0xFFFFFFFF)。
 * @param - len : 数字位数。
 * @param - size: 字体大小
 * @return 		: 无
 */
void lcd_showhexnum(unsigned short x, 
						unsigned short y, 
						unsigned int num, 
						unsigned char len,
						unsigned char size)
{         	
	...
	for(t = 0; t < len; t++)
	{
		...
		// 将0~9的数值转换为'0'~'9'字符，A~F转换为'A'~'F'字符
		if(temp < 10)
			lcd_showchar(x + (size / 2) * t, y, temp + '0', size, 0);
		else
			lcd_showchar(x + (size / 2) * t, y, temp - 10 + 'A', size, 0);
	}
}
