...
/**
 * @brief 像素时钟初始化
 * @param loopdiv	:	PLL5时钟设置，设置DIV_SELECT范围27~54
 * @param prediv	:	设置1~8分频
 * @param div		:	LCD_CLK = 24 * loopDiv / prediv / div
 */
...
void lcd_clear(unsigned int color) 
{
    ...
    // 遍历每个像素点，将其设置为指定颜色
    for (y = 0; y < tftlcd_dev.height; y++) {
        for (x = 0; x < tftlcd_dev.width; x++) {
...
void lcd_fill(unsigned short x0, unsigned short y0, unsigned short x1, unsigned short y1, unsigned int color) 
{
    ...
    if (x0 > x1) {
        unsigned short temp = x0;
        x0 = x1;
        x1 = temp;
    }
    if (y0 > y1) {
        unsigned short temp = y0;
        y0 = y1;
        y1 = temp;
    }
